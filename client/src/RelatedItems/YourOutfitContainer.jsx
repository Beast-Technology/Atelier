...
function YourOutfitContainer({productID, relatedItems, currentProduct}) {
  ...
  if (addButton) {
    addButtonDiv = (
      ...
      <button id="addButtonCard-button" onClick={(e)=>handleAddToOutfit(e)}>Add to Outfit</button>
      </div>
      </>
    )
  } else {
  ...
  return (
    ...
    {addButtonDiv}
    {
      (yourOutfitItems || []).map(yourOutfitItem=> {
        return <YourOutfitCard yourOutfitItem={yourOutfitItem} key={yourOutfitItem.id}/>
      })
    }
    </div>
    );
